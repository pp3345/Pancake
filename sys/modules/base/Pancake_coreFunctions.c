...
PHP_FUNCTION(setThread) {
	...
	if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "o|oal", &PANCAKE_GLOBALS(currentThread), &PANCAKE_GLOBALS(defaultVirtualHost), &virtualHostArray, &PANCAKE_GLOBALS(postMaxSize)) == FAILURE) {
		RETURN_FALSE;
	}
